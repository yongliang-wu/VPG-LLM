...
class LongVA:
    ...
    def load_video_all(self, video_path):
        if isinstance(video_path, str):
            ...
            vr = VideoReader(video_path[0], ctx=cpu(0))
        total_frame_num = len(vr)
        all_frames = vr.get_batch(range(total_frame_num)).asnumpy()
        print(f"all_frames: {all_frames.shape}")
        return all_frames  # (frames, height, width, channels)
...
def video_demo(model, video_info, num_sampled_frames):
    visual_path = os.path.join('data',video_info["video"])
    input_visuals = [visual_path]
    input_format = "Find the video segment that corresponds to the given textual query '{}' and determine its start and end frames."
    ...
    response = {"id": video_info["id"], "response": ""}
    try:
        print(f'Start to Generate for video id: {video_info["id"]}')
        prev = 0
        for x in model.stream_generate_until(query, gen_kwargs):
            output = json.loads(x.decode("utf-8").strip("\0"))["text"].strip()
            print(output[prev:], end="", flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--videobackedn", type=str, default="all")
    parser.add_argument("--device", type=str, default="cuda:0")
    args = parser.parse_args()
    model = LongVA(pretrained="./pretrained/Qwen2-LongVA-7B-DPO", model_name="llava_qwen", device_map=args.device, video_decode_backend="all")
    
    json_path = 'data/charades_sta_test.json'
    ...
    for video_info in video_list:
        response = video_demo(model, video_info, args.num_sampled_frames)
